...
보험설계사 인스타 피드용 카드뉴스를 자동 생성하고, 정해진 시간(KST 09:00 / 13:00 / 17:00, 하루 3회)에 자동 업로드하는 파이프라인.
...
│       ├── NOON/
│       └── PM/
├── captions/               # YYYY-MM-DD_AM.txt / _NOON.txt / _PM.txt
...
├── auto-approve.yml        # KST 06:30 / 10:30 / 14:30 → draft → approved
└── instagram-publish.yml   # KST 09:00 / 13:00 / 17:00 → 인스타 발행
```
...
```
cardnews/<YYYY-MM-DD>/<AM|NOON|PM>/<NN>
```
...
# LLM으로 일주일치 × AM/NOON/PM = 21세트
python generate.py --days 7 --topic "보험 상담 화법" --slots AM,NOON,PM --start 2026-05-08
```

생성물은 `output/YYYY-MM-DD/{AM|NOON|PM}/draft/01.png ... 0N.png`. 검수 후 `approved/` 로 이동해야 업로드 큐에 진입함.
...
- AM   슬롯: 게시 2.5h 전 = **KST 06:30**
- NOON 슬롯: 게시 2.5h 전 = **KST 10:30**
- PM   슬롯: 게시 2.5h 전 = **KST 14:30**
...
- 발행 워크플로가 실패하면 자동으로 GitHub Issue가 열립니다 (`[publish-failure] YYYY-MM-DD AM|NOON|PM`). repo Notifications 설정을 켜두면 이메일로 받음. 해결 후 이슈를 닫으면 됩니다. dry-run 실패 시에는 이슈를 만들지 않습니다.
...
2. **첫 생성 (Actions → "Generate cardnews" → Run workflow)**
   ...
   - 정상 생성: `use_llm = true`, `days = 7`, `slots = AM,NOON,PM`
...
7. 이후 cron이 매일 KST 06:30/10:30/14:30 자동 승인, 09:00/13:00/17:00 자동 발행.
