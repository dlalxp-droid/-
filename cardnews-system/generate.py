...
1) 이 세트가 예고하는 "결정적 한 문장/정답 멘트"는 딱 하나로 정하고, 그 문장을
   완성된 형태로는 카드 배열 어디에도 절대 쓰지 않는다 — 마지막 카드뿐 아니라
   before/after, turn, insight, tips, qa 등 중간에 나오는 어떤 kind 에도 해당.
   실제로 있었던 실패 사례: cover/summary 에서는 "그 한 문장 정확히 알려드릴게요"
   라고 예고해놓고, 정작 before/after 카드의 after 줄에 그 완성 문장을
   ("충분히 그럴 수 있어요. 결정 안 하시는 것도 결정이니까요." 처럼) 그대로 다
   써버림 — 이러면 카드만 봐도 답이 다 나와서 DM 할 이유가 없다.
   지킬 것:
   - before(틀린 예/실수)는 완전한 문장으로 구체적으로 보여줘도 된다 — 문제
     제기는 다 보여줄수록 좋다.
   - after(정답/해결)는 "방향·원리"까지만 준다. 예: 나쁜 예("충분히 그럴 수
     있어요. 결정 안 하시는 것도 결정이니까요." 전문 공개) 대신 좋은 예("핵심은
     반박 대신 인정, 그리고 대화를 열어두는 한마디를 붙이는 것 — 정확한 워딩은
     케이스별로 달라서 DM으로 드려요") 처럼 원리만 설명하고 완성 문장은 자른다.
   - bullets/tips/체크리스트도 마찬가지 — "무엇을 해야 하는지"의 카테고리는
     나열해도 되지만, 그대로 복붙해서 쓸 수 있는 완성된 문장·정확한 수치는 최소
     하나는 의도적으로 비운다 (나쁜 예: "톤 다운 / 어미 정리형 / 침묵 허용" 처럼
     실행 가능한 항목을 빠짐없이 완결).
   - qna/ranking 처럼 매 카드가 원래 완결된 답을 주는 구조에서도, 그 세트의
     "결정적 한 문장"에 해당하는 항목(보통 마지막 Q나 rank1)만은 이 규칙을
     반드시 적용한다 — 나머지 항목은 평소대로 완결해도 된다.
   "이 원칙은 맞는데, 정확히 뭐라고 말해야 하지?" 라는 궁금증이 세트를 다 봐도
   남아있어야 한다 — 정보를 아예 안 주는 게 아니라 "원리와 완성된 워딩 사이의
   gap"을 끝까지 유지하는 것.
