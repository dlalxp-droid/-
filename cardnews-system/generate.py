...
1) 이 세트가 예고하는 "결정적 한 문장/정답 멘트"는 딱 하나로 정하고, 그 문장의
   핵심 부분을 "OOO" 로 가려서 쓴다 (방송 자막에서 결정적 단어를 O로 가리는
   방식과 동일 — 한국 독자에게 아주 익숙한 궁금증 유발 장치라 말로 설명하는
   것보다 훨씬 확실하게 작동한다). 원칙만 설명하고 문장을 아예 자르는 것보다,
   문장은 끝까지 자연스럽게 완성하되 결정적 단어/구절 하나만 OOO 로 바꿔라.
   실제로 있었던 실패 사례: "충분히 그럴 수 있어요. 결정 안 하시는 것도
   결정이니까요." 를 after 카드에 완성 문장 그대로 다 써버림 — 카드만 봐도
   답이 다 나와서 DM 할 이유가 없었다.
   ...
   적용 범위: before/after, turn, insight, tips, qa, ranking 등 어떤 kind 에도
   해당 — 마지막 카드뿐 아니라 세트 중간에 나오는 "정답/해결" 성격의 카드라면
   전부 적용한다. before(틀린 예/실수)는 가릴 필요 없이 완전한 문장으로 구체적
   으로 보여줘도 된다 — 문제 제기는 다 보여줄수록 좋다.
   숫자·체크리스트도 같은 방식: "월말일수록 OOO초 이상 기다립니다" 처럼
   결정적 수치 하나를 OOO 로 가리거나, 체크리스트 항목 중 하나를 "OOO" 로
   대체한다 (나쁜 예 — 전부 공개: "톤 다운 / 어미 정리형 / 침묵 허용".
   좋은 예 — 하나 가림: "톤 다운 / OOO / 침묵 허용").
   qna/ranking 처럼 매 카드가 원래 완결된 답을 주는 구조에서도, 그 세트의
   "결정적 한 문장"에 해당하는 항목(보통 마지막 Q나 rank1)만은 이 OOO 마스킹을
   반드시 적용한다 — 나머지 항목은 평소대로 완결해도 된다.
   OOO 로 가린 부분은 cta 카드나 캡션에서 "그 OOO가 뭔지 궁금하면"처럼 다시
   한번 자연스럽게 언급해도 좋다 (단, 답을 흘리지는 말 것).
    ...
    return {
        ...
        "cards": [
            ...
            {"kind": "after",
             ...
             "after":  "“네, 충분히요. 다만 OOO만 한 가지 듣고 갈게요.”",
             "swipe": "왜 통할까"},
            {"kind": "insight",
             "eyebrow": "INSIGHT",
