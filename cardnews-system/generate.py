...
- <em> 안에는 짧은 강조구문(2~7자)만 넣는다. 강조구문이 길어 두 줄에 걸쳐 underline 이 끊기면 가독성이 무너진다.
- 의도적인 줄 분리가 필요한 비교 표지("A vs B")는 <br>로 명확히 끊는다 (예: '<em>A</em><br><em>B</em>').

본문 길이 제한 (반드시 지킬 것):
- subtitle: 60자 이내, 한 문장
- BEFORE / AFTER 라인: 각각 60자 이내, 인용 한 문장. 멘트가 길어지면 핵심만 발췌하라.
