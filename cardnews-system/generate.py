...
1) 카드 전체가 "왜 통하는지/무엇을 해야 하는지"는 확실히 알려주되, 마지막 내용
   카드(cta 바로 앞 1~2장)는 절대로 실행 가능한 체크리스트·정확한 문장·정확한
   숫자를 전부 완결해서 주지 않는다. 지금까지 자주 하던 실수:
   - 나쁜 예 (다 줌 — 이러면 댓글/DM 할 이유가 없음): "톤 다운 / 어미 정리형 /
     침묵 허용" 처럼 그대로 실행 가능한 항목을 빠짐없이 나열.
   - 좋은 예 (원칙은 주되 실행 디테일은 gap): "포인트는 속도 조절인데, 정확히
     어떤 어미로 바꾸는지는 고객 유형마다 달라서 케이스별로 다르게 잡아드려요."
   구체적으로: bullets/before/after/ref_body 에 "정확히 뭐라고 말해야 하는지"의
   실제 문장 전문, "정확히 몇 초/며칠"의 숫자, 체크리스트 3개를 전부 나열하는 걸
   마지막 1~2장에서는 하지 말고 그중 핵심 1개는 의도적으로 빼거나 "케이스마다
   다르다"로 뭉뚱그려라. qna/ranking 처럼 매 카드가 원래 완결된 답을 주는 구조여도
   최소 마지막 1개 항목(마지막 Q, rank1 등)만은 이 규칙을 적용해 완결하지 마라.
   "이 원칙은 맞는데, 내 케이스엔 정확히 어떻게 적용하지?" 라는 궁금증이 남게
   만드는 것이 목표 — 정보를 아예 안 주는 게 아니라 "원칙과 실행 디테일 사이의
   gap"을 남기는 것.
