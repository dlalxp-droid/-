...
PROMPT_DM_CTA_RULES = """## 낚시(호기심 갭) + DM 유도 CTA 규칙

이 계정의 전환 목표는 저장/댓글이 아니라 DM 상담 유입이다. 아래 두 가지를 지킨다:
...
2) 마지막 카드(kind: cta)는 저장/공유 유도가 아니라 DM 유도로 통일한다.
   핵심 문구는 "자세한 내용이 궁금하시면 DM 주세요" 계열의 의미를 반드시 담되,
   그대로 매번 복붙하지 말고 화자 말투·이번 토픽에 맞게 자연스럽게 표현을 바꿔라
   (예: "내 케이스엔 어떻게 적용하는지 궁금하면 DM 주세요" / "구체적인 멘트 버전은
   DM으로 알려드릴게요" / "이거 우리 상황이랑 똑같은데? 싶으면 DM 주세요").
   보험 상품 추천/가입 유도가 아니라 "이 화법/정보를 더 알고 싶으면"의 맥락을 유지한다
   (표시광고 규정 — 특정 상품 가입을 직접 권유하는 문구는 쓰지 않는다).
3) 캡션 마지막 CTA 줄도 같은 원칙 — "궁금하면 DM" 계열로 통일하되 표현은 매번 다르게.
"""
...
8) tips     — 실전 팁 2~3개
9) summary  — 한 줄 요약
10) cta     — DM 유도 (자세한 내용/내 케이스 맞춤 적용은 DM 유도, 저장·공유 아님)
"""
...
- 2~3문장 1인칭 회상 ("저도 처음엔…")
- DM 유도 한 줄 (궁금증 자극 질문 + "자세한 내용/내 케이스는 DM으로" 류 CTA)
- 마지막 줄 해시태그 5~10개
"""
...
def dummy_payload(topic: str, idx: int = 0) -> dict:
    ...
    return {
        ...
        "caption": (
            ...
            "내 상황엔 어떻게 적용하면 좋을지 궁금하면 DM 주세요 🙏\n"
            "\n"
        ...
        "cards": [
            ...
            {"kind": "cta",
             ...
             "subtitle": "고객 성향별로 톤이 달라져요. 궁금하면 DM 주세요.",
             "swipe": ""},
        ],
    }
